...
CUTOFF = datetime.strptime('2025-04-24_14-30-39', '%Y-%m-%d_%H-%M-%S')
...
def main():
    ...
    for folder in folders:
        # Open metadata
        metadata, output = _load_experiment(folder)
        if not metadata:
            continue

    instruction = (
        "You're goal is to tell whether the ground truth reason of corruption is in the answer or not. "
        "Please answer with 'yes' or 'no'.\n"
    )
    gt = 'Functional depedency between stlmt and wknck violated'
    instruction += 'The ground truth reason of corruption is: ' + gt + '\n'
    raise NotImplementedError('Needs to be finished')
    # instruction += 'The answer is: ' + str(answer) + '\n'

    print(llm_complete(instruction, provider='openai'))
    print(llm_complete(instruction, provider='gemini'))
