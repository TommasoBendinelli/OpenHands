...
AGENT_CLS_TO_FAKE_USER_RESPONSE_FN = {
    'CodeActAgentTicketsAnalysis': codeact_user_response,
}
...
def configure_app_for_evaluation(
    ...
    metadata.agent_class = 'CodeActAgentTicketsAnalysis'
...
def initialize_runtime(
    ...
    base_path = Path('evaluation/benchmarks/schurter/tasks/datasources/')

    if 'MachineDeviceManuals' in prompt_metadata['datasources']:
        raise NotImplementedError(
            'MachineDeviceManuals datasource is not implemented yet.'
        )

    if 'MachineDocumentation' in prompt_metadata['datasources']:
        raise NotImplementedError(
            'MachineDocumentation datasource is not implemented yet.'
        )

    if 'MachineMaintenanceLog' in prompt_metadata['datasources']:
        data_path = base_path / 'MachineMaintenanceLog'
        for file in (data_path).glob('*'):
            runtime.copy_to(file, '/workspace/MachineMaintenanceLog')

    if 'MachineProcessData' in prompt_metadata['datasources']:
        data_path = base_path / 'MachineProcessData'
        for file in (data_path).glob('*'):
            runtime.copy_to(file, '/workspace/MachineProcessData')

    if 'MachineProcessDataDoc' in prompt_metadata['datasources']:
        data_path = base_path / 'MachineProcessDataDoc'
        for file in (data_path).glob('*'):
            runtime.copy_to(file, '/workspace/MachineProcessDataDoc')

    if 'MachineProcessDescriptions' in prompt_metadata['datasources']:
        raise NotImplementedError(
            'MachineProcessDescriptions datasource is not implemented yet.'
        )

    if 'MachineSettings' in prompt_metadata['datasources']:
        data_path = base_path / 'MachineSettings'
        for file in (data_path).glob('*'):
            runtime.copy_to(file, '/workspace/MachineSettings')

    if 'MachineSpecificHowTos' in prompt_metadata['datasources']:
        data_path = base_path / 'MachineSpecificHowTos'
        for file in (data_path).glob('*'):
            runtime.copy_to(file, '/workspace/MachineSpecificHowTos')

    if 'MachineSpecificUserManual' in prompt_metadata['datasources']:
        data_path = base_path / 'MachineSpecificUserManual'
        for file in (data_path).glob('*'):
            runtime.copy_to(file, '/workspace/MachineSpecificUserManual')

    if 'WorkInstructions' in prompt_metadata['datasources']:
        data_path = base_path / 'WorkInstructions'
        for file in (data_path).glob('*'):
            runtime.copy_to(file, '/workspace/WorkInstructions')
...
def process_instance(
    ...
    initialize_runtime(runtime, instance, prompt_metadata, cfg=cfg)
