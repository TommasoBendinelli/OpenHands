...
def generate_valley_signal(n_valleys, length=100, width=0.02):
    """
    Generate a synthetic signal containing n_valleys Gaussian dips (valleys).

    :param n_valleys: int, number of valleys to embed
    :param length: int, number of time points
    :param width: float, standard deviation of each Gaussian dip
    :return: 1D numpy array of shape (length,)
    """
    t = np.linspace(0, 1, length)
    x = np.zeros_like(t)
    centers = np.linspace(0.1, 0.9, n_valleys)
    for c in centers:
        x -= np.exp(-((t - c) ** 2) / (2 * width**2))  # Notice the minus sign
    return x
...
def create_dataset(
    ...
    length=100,
    width=0.02,
    output_path='time_series_dataset.csv',
):
    """
    Build a CSV dataset of signals labeled by their true number of valleys.
    ...
    data = []
    labels = []
    for k in range(1, max_valleys + 1):
        for _ in range(samples_per_class):
            x = generate_valley_signal(k, length, width)
            data.append(x)
            labels.append(k)
...
if __name__ == '__main__':
    ...
    for i in range(len(df) - 1, 1, -1):
        ...
        plt.title(f'Number of Valleys: {df.iloc[i, -1]}')
