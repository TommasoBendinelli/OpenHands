...
def combined_outlier_ratio_plot(train_df: pd.DataFrame,
    ...
    # histogram counts
    tr0, _ = np.histogram(tr_ratio[tr_lbl == 0], bins=bins)
    tr1, _ = np.histogram(tr_ratio[tr_lbl == 1], bins=bins)
    te0, _ = np.histogram(te_ratio[te_lbl == 0], bins=bins)
    te1, _ = np.histogram(te_ratio[te_lbl == 1], bins=bins)
