...
def combined_sanity_plot(
    ...
    # common bins
    bins = np.linspace(
        min(diff_tr.min(), diff_te.min()), max(diff_tr.max(), diff_te.max()), 30
    )
...
if __name__ == '__main__':
    ...
    plot = combined_sanity_plot(
        train_df, test_df, '', out_dir / 'delta_distribution_combined.png'
    )
    plot.savefig(out_dir / 'delta_distribution_combined.png', dpi=150)

    # Save also in the common folder if FIGS_DIR is provided
    figs_dir = Path(
        os.environ.get('FIGS_DIR', '67ff59b20231d9f95909f426/figs/datasets')
    )
    plot.savefig(figs_dir / 'dominant_feature.png', dpi=150)
    plot.savefig(figs_dir / 'dominant_feature.pdf', dpi=150)
