...
def main():
    ...
    for folder in runs:
        ...
        if (
            outputs[0]['error']
            and "RequestHTTPError: Server error '500 Internal Server Err"
            in outputs[0]['error']
        ):
            continue
        ...
        IPythonActions = [
            x
            for x in outputs[0]['history']
            if 'observation' in x and x['observation'] == 'run_ipython'
        ]
        ...
        AssistantShellActions = [
            x
            for x in outputs[0]['history']
            if 'observation' in x and x['observation'] == 'run'
        ]
        ...
        if "raise Exception('sklearn is disabled!')" in str(IPythonActions):
            before = str(IPythonActions).split(
        ...
        if "raise Exception('you are not allowed to use pd.read_csv!')" in str(
            IPythonActions
        ):
            before = str(IPythonActions).split(
    ...
    results_all['number_of_submissions_when_right'] = results_all.apply(
        lambda x: x['number_of_submissions'] if x['perfect_accuracy'] == 1 else np.nan,
        axis=1,
    )
    ...
    results_all['cost_when_right'] = results_all.apply(
        lambda x: x['final_accumulated_cost'] if x['perfect_accuracy'] == 1 else np.nan,
        axis=1,
    )
    ...
    experimental_runs = results_all.loc[
        results_all['identifier_experiment'].isin(identifiers)
    ]
    ...
    experimental_runs = experimental_runs[
        ...
        + [
            col
            for col in experimental_runs.columns
            if col not in ['error', 'best_metric']
        ]
    ]
    ...
    experimental_runs = experimental_runs.loc[
        ~(
            (experimental_runs['llm_config'] == 'gpt-o4-mini')
            & (experimental_runs['max_budget_per_task'] != 1)
        )
    ]
    experimental_runs = experimental_runs.loc[
        ~(
            (experimental_runs['llm_config'] == 'gemini_pro_pro')
            & (experimental_runs['max_budget_per_task'] != 1)
        )
    ]
    experimental_runs = experimental_runs.loc[
        ~(
            (experimental_runs['llm_config'] == 'open_router_claude')
            & (experimental_runs['max_budget_per_task'] != 1)
        )
    ]

    to_be_replaced = experimental_runs.loc[
        ~(
            (experimental_runs['llm_config'] == 'gpt-o4-mini')
            & (experimental_runs['identifier_experiment'] == 'baseline')
        )
    ]
    to_be_added = experimental_runs.loc[
        (
            experimental_runs['llm_config'].isin(['gpt-o4-mini', 'gpt-41'])
            & (
                experimental_runs['identifier_experiment']
                == 'baseline_native_tool_calling'
            )
        )
    ]
    ...
    experimental_runs[experimental_runs['llm_config'] == 'gpt-o4-mini'].sort_values(
        by=['instance']
    )
    experimental_runs[experimental_runs['llm_config'] == 'gemini_pro'].sort_values(
        by=['instance']
    )

    experimental_runs = pd.concat([to_be_replaced, to_be_added], ignore_index=True)
    experimental_runs.loc[
        (experimental_runs['llm_config'] == 'gpt-41')
        & (experimental_runs['best_metric'] == 1)
    ]
    ...
    experimental_runs = experimental_runs.loc[
        ~(
            (experimental_runs['identifier_experiment'] == 'baseline')
            & (experimental_runs['is_plotting_enabled'] == False)
        )
    ]
    experimental_runs = experimental_runs.loc[
        ~(
            (experimental_runs['identifier_experiment'] == 'plot_disabled')
            & (experimental_runs['is_plotting_enabled'] == True)
        )
    ]
    ...
    experimental_runs = experimental_runs.loc[
        ~(
            (experimental_runs['llm_config'] == 'gpt-o4-mini')
            & (experimental_runs['native_tool_calling'] == False)
        )
    ]
    ...
    for identifier in identifiers:
        for model in models + ['llama']:
            for instance in datasets:
                mask = (
                    (experimental_runs['llm_config'] == model)
                    & (experimental_runs['identifier_experiment'] == identifier)
                    & (experimental_runs['instance'] == instance)
                )
                other_runs = experimental_runs.loc[mask]
                # Compute perfect_accuracy_all_the_time
                is_always_perfect = other_runs['perfect_accuracy'].sum() == len(
                    other_runs
                )

                # Compute almost_perfect_accuracy_all_the_time
                is_always_almost_perfect = other_runs[
                    'almost_perfect_accuracy'
                ].sum() == len(other_runs)
                is_anything_perfect = other_runs['perfect_accuracy'].sum() > 0
                is_anything_almost_perfect = (
                    other_runs['almost_perfect_accuracy'].sum() > 0
                )
                is_anything_perfect_at_first = (
                    other_runs['results_got_right_at_first'].sum() > 0
                )
                # Check if there is at least one perfect accuracy
                # Add this back to the dataframe
                experimental_runs.loc[mask, 'perfect_accuracy_all_the_time'] = (
                    1 if is_always_perfect else 0
                )
                experimental_runs.loc[mask, 'almost_perfect_accuracy_all_the_time'] = (
                    1 if is_always_almost_perfect else 0
                )
                experimental_runs.loc[mask, 'is_anything_perfect'] = (
                    1 if is_anything_perfect else 0
                )
                experimental_runs.loc[mask, 'is_anything_almost_perfect'] = (
                    1 if is_anything_almost_perfect else 0
                )
                experimental_runs.loc[mask, 'is_anything_perfect_at_first'] = (
    ...
    for scenario in identifiers:
        for model in models:
            ...
            #     breakpoint()
            other_runs = experimental_runs.loc[
                (experimental_runs['llm_config'] == model)
                & (experimental_runs['identifier_experiment'] == scenario)
            ]
            ...
            table1_list.append(
                {
                    ...
                    'mean_perfect_accuracy': group_df['perfect_accuracy']
                    .fillna(0)
                    .mean(),
                    'mean_almost_perfect_accuracy': group_df[
                        'almost_perfect_accuracy'
                    ].mean(),
                    'mean_perfect_accuracy_all_the_time': group_df[
                        'perfect_accuracy_all_the_time'
                    ].mean(),
                    'mean_almost_perfect_accuracy_all_the_time': group_df[
                    ...
                    'perfect_accuracy_at_first': group_df[
                        'results_got_right_at_first'
                    ].mean(),
    ...
    def table_2(
        df,
        models: List[str],
        identifier_considered: List[str] = ['baseline', 'plot_disabled'],
    ):
        """ """
    ...
    invalid = experimental_runs['is_cheating'] & (experimental_runs['perfect_accuracy'])
    assert (
        not invalid.any()
    ...
    filled_1 = fill_the_table(
        entry,
        important_columns=[
            'perfect_accuracy_at_first',
            'mean_perfect_accuracy',
            'mean_almost_perfect_accuracy',
        ],
    )
    ...
    percentage_correct_at_first = fill_the_table(
        entry,
        important_columns=[
            'perfect_accuracy_at_first',
            'number_of_python_calls_when_right_at_first',
    ...
    filled = fill_the_table(
        entry,
        important_columns=[
            'perfect_accuracy_at_first',
            'mean_almost_perfect_accuracy_all_the_time',
        ],
    )
    breakpoint()
    filled_2 = fill_the_table(
    ...
    entry_3 = table_1(
        experimental_runs, identifier_experiment='constraint', models=models
    )
    filled = fill_the_table(
        entry_3,
        important_columns=['mean_perfect_accuracy', 'mean_almost_perfect_accuracy'],
    )

    entry_2 = table_1(
        experimental_runs, identifier_experiment='plot_disabled', models=models
    )
    filled = fill_the_table(
        entry_2,
        important_columns=['mean_perfect_accuracy', 'mean_almost_perfect_accuracy'],
    ...
    def build_win_tie_loss(
        ...
        for task_name, group in grouped:
            ...
            try:
                acc_a = group.loc[
                    group['identifier_experiment'] == exp_a, 'almost_perfect_accuracy'
                ].values[0]
                acc_b = group.loc[
                    group['identifier_experiment'] == exp_b, 'almost_perfect_accuracy'
                ].values[0]
            except IndexError:
